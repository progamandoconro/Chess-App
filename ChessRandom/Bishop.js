        ...
        const bishopMove = () => {
            ...
            if (Number(Math.random()) > 0.49) {
                for (let index = 0; index < allMoves2.length; index++) {
                    const cond = diagonal.some((val) => allMoves2[index].indexOf(val) !== -1);
                    if (cond === true) {
                        const d = allMoves2[index];
                ...
                for (let index = 0; index < allMoves.length; index++) {
                    const cond = diagonal.some((val) => allMoves[index].indexOf(val) !== -1)
                    if (cond === true) {
                        const d = allMoves[index];
                        const n = Math.floor(Math.random() * d.length);
